...
function checkUserSession() {
  ...
  const status = document.getElementById('portal-user-status');
  if (user && pill) {
  ...
  if (status) {
    if (user) {
      status.textContent = `Logged in as ${user.fname} ${user.lname} · ${user.email}`;
    } else {
      status.textContent = 'Not signed in — please use auth.html to login.';
    }
  }
}
...
// MY ACCOUNT SECTION
// ============================================================
function renderMyAccount(orderFilter = null) {
  ...
  const filteredOrders = orderFilter ? orders.filter(o => o.category === orderFilter) : orders;
  ...
  el.innerHTML = `
    ...
    ${tickets.length ? tickets.map(renderOrderRow).join('') : `
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:2.5rem;text-align:center;margin-bottom:2rem;color:var(--text-muted)">
        ...
        <button onclick="showAccountTab('tickets')" style="margin-top:1rem;background:var(--accent-gold);color:#000;border:none;padding:8px 20px;border-radius:6px;font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:0.85rem;cursor:pointer;letter-spacing:1px">BOOK NOW →</button>
      </div>`}

    <!-- ORDERS -->
    <div id="myaccount-orders" class="section-title-row" style="margin-bottom:1rem;margin-top:1.5rem">
      <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <div>
          <h2 class="section-title" style="font-size:1.2rem">📦 MY ORDERS (${orders.length})</h2>
          ${orderFilter ? `<div style="color:var(--accent-gold);font-size:0.88rem;margin-top:8px">Showing ${orderFilter} purchases only</div>` : ''}
        </div>
      </div>
    </div>
    ${filteredOrders.length ? filteredOrders.map(renderOrderRow).join('') : `
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:2.5rem;text-align:center;color:var(--text-muted)">
        <div style="font-size:2.5rem;margin-bottom:0.75rem">🛍️</div>
        <div>No ${orderFilter?orderFilter.toLowerCase():'orders'} found yet</div>
...
function showAccountTab(section) {
  switchSection('myaccount', document.querySelector('[data-section=myaccount]'));
  if (section === 'tickets') {
    renderMyAccount();
    setTimeout(() => document.getElementById('myaccount-tickets')?.scrollIntoView({behavior:'smooth', block:'start'}), 100);
    return;
  }
  if (section === 'Jersey' || section === 'Merchandise') {
    renderMyAccount(section);
    setTimeout(() => document.getElementById('myaccount-orders')?.scrollIntoView({behavior:'smooth', block:'start'}), 100);
    return;
  }
  renderMyAccount();
}
...
function closePopupMenu() {
  ...
  if (!center) return;
  center.classList.remove('popup-open');
  document.removeEventListener('click', handleMenuOutsideClick, true);
}

function handleMenuOutsideClick(e) {
  const center = document.querySelector('.nav-center');
  const menuBtn = document.querySelector('.hamburger');
  if (!center || !menuBtn) return;
  if (center.contains(e.target) || menuBtn.contains(e.target)) return;
  closePopupMenu();
}
...
function toggleMenu() {
  const center = document.querySelector('.nav-center');
  if (!center) return;
  if (center.classList.contains('popup-open')) {
    closePopupMenu();
    return;
  }
  center.classList.add('popup-open');
  setTimeout(() => {
...
window.addEventListener('resize', () => {
  if (window.innerWidth > 900) closePopupMenu();
});
...
function renderBowlingCharts() {
  ...
  makeChart('chart-economy','line', eco.map(x=>x[0].split(' ').pop()), [{
    label:'Economy', data:eco.map(x=>x[1]),
    borderColor:'rgba(14,165,233,0.9)',
    backgroundColor:'rgba(14,165,233,0.15)',
    borderWidth:2, tension:0.35, fill:true, pointRadius:5, pointBackgroundColor:'#0ea5e9', pointBorderColor:'#ffffff', pointBorderWidth:2
  }], { yMin:5, extra:{ plugins:{ legend:{display:false} } } });
...
function renderTeamCharts() {
  ...
  }], { yMin:75, extra:{ indexAxis:'x', plugins:{ legend:{display:false} } } });
