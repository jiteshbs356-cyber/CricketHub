...
const MERCHANDISE=[
  {id:'mc1', cat:'bats',        imgAlt:'🏏',name:'Virat Kohli Slazenger V500 Bat',          team:'RCB', price:4999, mrp:6999, desc:'Slazenger V500 English Willow bat with Virat Kohli autograph & India flag print',     limited:true },
  {id:'mc2', cat:'bats',        imgAlt:'🏏',name:'Rohit Sharma SS Vintage Bat',             team:'MI',  price:5499, mrp:7499, desc:'SS Vintage English Willow Grade 1 bat — Rohit Sharma signature edition, display ready',limited:true },
  {id:'mc3', cat:'bats',        imgAlt:'🏏',name:'CEAT Hitman Edition Bat',                 team:'MI',  price:1999, mrp:2799, desc:"Rohit Sharma's CEAT Hitman bat — full size with all-terrain grip, match standard",     limited:false},
  {id:'mc4', cat:'bats',        imgAlt:'🏏',name:'MRF Genius Grand Edition Bat',            team:'CSK', price:3499, mrp:4999, desc:'MRF Genius Grand Edition game-used bat with authentic player signature',               limited:true },
  {id:'mc5', cat:'signed',      imgAlt:'✍️',name:'Virat Kohli Signed India Jersey (Framed)',team:'RCB', price:24999,mrp:29999,desc:'India blue jersey signed by Virat Kohli — premium display frame with player photo',     limited:true },
  {id:'mc6',cat:'signed',      imgAlt:'✍️',name:'MS Dhoni SG Savage WK Gloves',           team:'CSK', price:18999,mrp:24999,desc:"Dhoni's SG Savage wicket-keeping gloves, hand-signed with certificate of authenticity",  limited:true },
  {id:'mc7',cat:'signed',      imgAlt:'✍️',name:'Jasprit Bumrah Signed Match Ball',        team:'MI',  price:7999, mrp:10999,desc:'Official white match cricket ball signed by Jasprit Bumrah with certificate',           limited:true },
  {id:'mc8',cat:'signed',      imgAlt:'✍️',name:'Rohit Sharma Signed Bat (Framed)',        team:'MI',  price:21999,mrp:27999,desc:'SS Vintage bat signed by Rohit Sharma in premium display frame with certificate',       limited:true },
  {id:'mc9',cat:'accessories', imgAlt:'⌚',name:'IPL Smart Wristband 2026',                team:null,  price:1299, mrp:1799, desc:'Smart fitness wristband with IPL team themes, heart rate & step tracking',             limited:false},
  {id:'mc10',cat:'accessories', imgAlt:'🎒',name:'MI Official Backpack 30L',               team:'MI',  price:1999, mrp:2799, desc:'30L official MI backpack, padded laptop sleeve, water bottle pocket, premium build',    limited:false},
  {id:'mc11',cat:'accessories', imgAlt:'🕶️',name:'Cricket UV400 Sports Sunglasses',        team:'RCB', price:899,  mrp:1299, desc:'UV400 polarised blue mirror sports sunglasses, wraparound frame for stadium cricket',   limited:false},
  {id:'mc12',cat:'accessories', imgAlt:'🧣',name:'CSK Official Fan Scarf',                  team:'CSK', price:599,  mrp:799,  desc:'Official CSK yellow & blue fan scarf — "Chennai Super Kings" printed text, fringe ends',limited:false},
  {id:'mc13',cat:'collectibles',imgAlt:'🏆',name:'IPL Trophy Replica Set (3 Sizes)',        team:null,  price:3499, mrp:4999, desc:'Set of 3 gold-plated IPL trophy replicas — large, medium, small with blue ribbons',    limited:true },
  {id:'mc14',cat:'collectibles',imgAlt:'🪆',name:'MS Dhoni Cricketer Bobblehead',          team:'CSK', price:1299, mrp:1799, desc:'Hand-painted MS Dhoni bobblehead in India blue kit with bat & ball, 18cm figurine',    limited:true },
  {id:'mc15',cat:'collectibles',imgAlt:'🪆',name:'Virat Kohli Bobblehead Figurine',        team:'RCB', price:1299, mrp:1799, desc:'Virat Kohli collectible figurine — India jersey, sitting pose on "VIRAT KOHLI" base',   limited:true },
  {id:'mc16',cat:'collectibles',imgAlt:'📸',name:'IPL All Team Fan Paddle Set',             team:null,  price:1799, mrp:2499, desc:'Set of 10 IPL team fan paddles — all teams with logos, perfect for stadium support',    limited:false},
];

// ── Merch image map — local uploaded real product images ────────
const MERCH_IMG_MAP = {
  mc1:  'images/merch/mc5_vk_bat.png',
  mc2:  'images/merch/mc6_rohit_bat.jpg',
  mc3:  'images/merch/mc7_mini_bat.jpg',
  mc4:  'images/merch/mc8_dhoni_bat.png',
  mc5:  'images/merch/mc9_vk_signed_jersey.jpg',
  mc6:  'images/merch/mc10_dhoni_gloves.jpg',
  mc7:  'images/merch/mc11_bumrah_ball.jpg',
  mc8:  'images/merch/mc12_rohit_signed_bat.png',
  mc9:  'images/merch/mc13_wristband.jpg',
  mc10: 'images/merch/mc14_backpack.jpg',
  mc11: 'images/merch/mc15_sunglasses.jpg',
  mc12: 'images/merch/mc16_scarf.jpg',
  mc13: 'images/merch/mc17_trophy.jpg',
  mc14: 'images/merch/mc18_dhoni_bobble.jpg',
  mc15: 'images/merch/mc19_vk_bobble.jpg',
  mc16: 'images/merch/mc20_photobook.jpg',
};
...
function renderCartItems(){
  ...
  if(couponNotice){
    couponNotice.style.display = activeCoupon ? 'block' : 'none';
    couponNotice.innerHTML = activeCoupon ? `Coupon <strong>${activeCoupon}</strong> applied` : '';
  }
...
function applyCoupon(){
  ...
  if(!code){showToast('⚠️ Enter a coupon code first.','error');return;}
  if(COUPONS[code]){
    activeCoupon=code;
    showToast(`🎉 Coupon ${code} applied!`,'success');
    renderCartItems();
  } else {
    showToast('❌ Invalid. Try: IPL25, CRICKET10, FLAT500, NEWUSER','error');
  }
}
...
function setBuyNowModalContent(html) {
  const el = document.getElementById('buyNow-modal-inner');
  if (!el) return;
  el.innerHTML = html;
}
...
function renderSizePicker(){
  ...
  const imgUrl = (j.specialImg || JERSEY_IMG[j.team] || '') + '?nocache=' + j.id + '_v28';
  const sizeButtons = j.sizes.map(s => {
    const selected = s === sizePick.size ? ' selected' : '';
    return `<button class="size-pick-btn${selected}" onclick="spSelectSize('${s}')">${s}</button>`;
  }).join('');
  const totalPrice = j.price * sizePick.qty;
  const delivery = totalPrice >= 999 ? 'FREE' : '₹49';
  const totalAmount = (totalPrice + (totalPrice < 999 ? 49 : 0)).toLocaleString();

  setBuyNowModalContent(`
    <div class="tbm-header">
      ...
      <div class="tbm-match-title" style="font-size:1.2rem;display:flex;gap:0.75rem;align-items:center">
        <div style="width:60px;height:60px;background:#060f1f;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          <img src="${imgUrl}" alt="${j.name}"
            style="width:100%;height:100%;object-fit:cover"
            onerror="this.outerHTML='<span style=\"font-size:2rem\">${j.imgAlt}</span>'">
        </div>
        ${j.name}
      </div>
    ...
    <div class="pay-summary-box">
      <div class="pay-row"><span>${j.name} · Size ${sizePick.size} × ${sizePick.qty}</span><span>₹${totalPrice.toLocaleString()}</span></div>
      <div class="pay-row"><span>Delivery</span><span style="color:var(--accent-green)">${delivery}</span></div>
      <div class="pay-row grand"><span>TOTAL</span><span>₹${totalAmount}</span></div>
    </div>
...
function renderMerchPicker(){
  ...
  const totalPrice = m.price * merchPick.qty;
  const delivery = totalPrice >= 999 ? 'FREE' : '₹49';
  const totalAmount = (totalPrice + (totalPrice < 999 ? 49 : 0)).toLocaleString();
  ...
  setBuyNowModalContent(`
      ...
      <div class="tbm-match-title" style="font-size:1.2rem;display:flex;gap:0.75rem;align-items:center">
        <div style="width:60px;height:60px;background:#060f1f;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          <img src="${imgUrl}" alt="${m.name}"
            style="width:100%;height:100%;object-fit:cover"
            onerror="this.outerHTML='<span style=\"font-size:2rem\">${m.imgAlt}</span>'">
        </div>
        ${m.name}
      </div>
    ...
    <div class="pay-summary-box">
      <div class="pay-row"><span>${m.name} × ${merchPick.qty}</span><span>₹${totalPrice.toLocaleString()}</span></div>
      <div class="pay-row"><span>Delivery</span><span style="color:var(--accent-green)">${delivery}</span></div>
      <div class="pay-row grand"><span>TOTAL</span><span>₹${totalAmount}</span></div>
    </div>
    <div style="display:flex;gap:8px;margin-top:0.5rem">
...
function openBuyNow(type,itemId){
  ...
  document.getElementById('buyNow-modal').classList.add('open');
  document.body.style.overflow='hidden';
  // Add validation after modal is rendered
...
function renderBuyNow(){
  ...
  if(step===1){
    const imgUrl=type==='jersey'?(item.specialImg||JERSEY_IMG[item.team]||''):type==='merch'?(MERCH_IMG_MAP[item.id]||''):'';
    body=`
      <div style="display:flex;gap:1rem;align-items:flex-start;background:var(--bg-deep);border-radius:8px;padding:1rem;margin-bottom:1rem">
        ${imgUrl?`<div style="width:100px;height:100px;background:#060f1f;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          <img src="${imgUrl}?nocache=${item.id}_v28" alt="${item.name}"
            style="width:100%;height:100%;object-fit:cover"
            onerror="this.outerHTML='<span style=\\'font-size:2.5rem\\'>${item.imgAlt}</span>'">
        </div>`:`<span style="font-size:3rem">${item.img||item.imgAlt}</span>`}
    ...
    body=`
      ...
      <div style="display:flex;gap:8px"><button class="tbm-add-cart-btn" style="background:var(--bg-card3);color:var(--text-secondary);flex:0.4" onclick="bnStep(1)">← BACK</button><button class="tbm-add-cart-btn" style="flex:1" onclick="bnStep(3)">PAY ₹${grand.toLocaleString()} SECURELY →</button></div>`;
  } else if(step===3){
    body=`
      <div class="tbm-section-title">SECURE PAYMENT</div>
      ...
      <div class="pay-summary-box" style="margin-top:1rem">
        <div class="pay-row"><span>${item.name}${size?' · '+size:''} × ${qty}</span><span>₹${total.toLocaleString()}</span></div>
        <div class="pay-row"><span>Delivery</span><span style="color:var(--accent-green)">${delivery===0?'FREE':'₹'+delivery}</span></div>
        <div class="pay-row grand"><span>TOTAL</span><span>₹${grand.toLocaleString()}</span></div>
      </div>
      <div style="color:var(--text-muted);font-size:0.85rem;margin:0.75rem 0">Open the secure payment modal shown in the previous design to complete this purchase.</div>
      <div style="display:flex;gap:8px"><button class="tbm-add-cart-btn" style="background:var(--bg-card3);color:var(--text-secondary);flex:0.4" onclick="bnStep(2)">← BACK</button><button class="tbm-add-cart-btn" style="flex:1" onclick="openBuyNowPayment()">🔒 PAY ₹${grand.toLocaleString()} SECURELY →</button></div>`;
  } else if(step===4){
  ...
  setBuyNowModalContent(`${step<4?hdr:''}<div class="checkout-steps" style="margin-bottom:1.5rem">${stepsHtml}</div>${body}`);
  setTimeout(addInputValidation, 100);
}
...
window.bnStep=(s)=>{
  if (s === 3) {
    // Validate form fields before proceeding to payment
    clearFieldErrors();
    ...
    const fnameError = validateName(fname);
    if (fnameError) {
      showFieldError('bn-fname', fnameError);
      hasError = true;
    }
    
    const lnameError = validateName(lname);
    if (lnameError) {
      showFieldError('bn-lname', lnameError);
      hasError = true;
    }
    
    const emailError = validateEmail(email);
    if (emailError) {
      showFieldError('bn-email', emailError);
      hasError = true;
    }
    
    const phoneError = validatePhone(phone);
    if (phoneError) {
      showFieldError('bn-phone', phoneError);
      hasError = true;
    }
    
    if (!addr) {
      showFieldError('bn-addr', '⚠️ This field is required.');
      hasError = true;
    }
...
window.openBuyNowPayment = ()=>{
  const item = buyNowState.item;
  if(!item) return;
  const total = item.price * buyNowState.qty;
  const delivery = total < 999 ? 49 : 0;
  const grand = total + delivery;
  if(typeof window.openPaymentModal==='function'){
    openPaymentModal(grand,'IPL Purchase',()=>{
      buyNowState.step=4;
      renderBuyNow();
    });
  } else {
...
window.openTicketPayment = ()=>{
  const m = IPL_MATCHES.find(x=>x.id===ticketState.matchId);
  if(!m) return;
  const selPrice = m.prices.find(p=>p.stand===ticketState.stand);
  const total = (selPrice?.price||0) * ticketState.qty;
...
function renderTicketModal(){
  ...
  if(step===1){
    ...
    body=`
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="tbm-add-cart-btn" style="flex:1" onclick="tsStep(4)">PAY ₹${grand.toLocaleString()} SECURELY →</button>
      </div>`;
  }
  ...
  setBuyNowModalContent(`
    ...
    ${body}`);
  if (step === 3) setTimeout(addInputValidation, 100);
}
...
window.tsStep=(s)=>{
  if (s === 4) {
    ...
    const fnameError = validateName(fname);
    if (fnameError) { showFieldError('ts-fname', fnameError); hasError = true; }

    const lnameError = validateName(lname);
    if (lnameError) { showFieldError('ts-lname', lnameError); hasError = true; }

    const emailError = validateEmail(email);
    if (emailError) { showFieldError('ts-email', emailError); hasError = true; }

    const phoneError = validatePhone(phone);
    if (phoneError) { showFieldError('ts-phone', phoneError); hasError = true; }

    if (hasError) return;
    showToast('✅ Booking details confirmed. Opening secure payment…','success');
    openTicketPayment();
    return;
  }
  ticketState.step=s;
  if(s===2) ticketState.seatStep=1;
  renderTicketModal();
};
...
function renderCheckoutModal(){
  ...
  if(checkoutStep===1){
    ...
    body=`<h3 style="font-family:Oswald,sans-serif;font-size:1.3rem;margin-bottom:1rem;letter-spacing:1px">PAYMENT</h3>
    <div class="pay-summary-box" style="margin-top:1rem">
    ...
    <div style="color:var(--text-muted);font-size:0.85rem;margin:0.75rem 0">Click PAY to open the secure payment modal and complete your order.</div>
    <div style="display:flex;gap:8px">
      ...
      <button class="checkout-btn" style="flex:1" onclick="openCheckoutPayment()">🔒 PAY ₹${total.toLocaleString()} →</button>
    </div>`;
...
window.goCheckoutStep=(s)=>{
  if (s === 2) {
    ...
    const fnameError = validateName(fname);
    if (fnameError) {
      showFieldError('co-fname', fnameError);
      hasError = true;
    }
    
    const lnameError = validateName(lname);
    if (lnameError) {
      showFieldError('co-lname', lnameError);
      hasError = true;
    }
    
    const emailError = validateEmail(email);
    if (emailError) {
      showFieldError('co-email', emailError);
      hasError = true;
    }
    
    const phoneError = validatePhone(phone);
    if (phoneError) {
      showFieldError('co-phone', phoneError);
      hasError = true;
    }
    
    if (!addr) {
      showFieldError('co-addr', '⚠️ This field is required.');
      hasError = true;
    }
...
function validateName(name) {
  if (!name) return '⚠️ This field is required.';
  if (!/^[a-zA-Z\s]+$/.test(name)) return '⚠️ Should Enter Alphabet only.';
  return null;
}

function validatePhone(phone) {
  if (!phone) return '⚠️ This field is required.';
  const normalized = phone.replace(/\s+/g, '');
  const digits = normalized.replace(/\D/g, '');
  if (!/^[+]?\d+$/.test(normalized)) return '⚠️ Should Enter Only Number.';
  if (digits.length < 10) return '⚠️ Enter 10 Phone Number.';
  if (digits.length > 15) return '⚠️ Phone number too long.';
  return null;
}

function validateEmail(email) {
  if (!email) return '⚠️ This field is required.';
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return '⚠️ Please enter a valid email address.';
  return null;
}
...
function addInputValidation() {
  // Buy Now modal validation
  const bnFname = document.getElementById('bn-fname');
  const bnLname = document.getElementById('bn-lname');
  const bnPhone = document.getElementById('bn-phone');
  
  if (bnFname) {
    bnFname.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^a-zA-Z\s]/g, '');
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('bn-fname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('bn-fname', error);
      } else {
        clearFieldError('bn-fname');
      }
    });
  }
  
  if (bnLname) {
    bnLname.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^a-zA-Z\s]/g, '');
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('bn-lname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('bn-lname', error);
      } else {
        clearFieldError('bn-lname');
      }
    });
  }
  
  if (bnPhone) {
    bnPhone.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^0-9+]/g, '');
      const normalized = cleaned[0] === '+' ? '+' + cleaned.slice(1).replace(/\+/g, '') : cleaned.replace(/\+/g, '');
      if (normalized !== this.value) {
        this.value = normalized;
        showFieldError('bn-phone', '⚠️ Should Enter Only Number.');
        return;
      }
      const digits = this.value.replace(/\D/g, '');
      if (digits.length > 0 && digits.length < 10) {
        showFieldError('bn-phone', '⚠️ Enter 10 Phone Number.');
        return;
      }
      const error = validatePhone(this.value.trim());
      if (error) {
        showFieldError('bn-phone', error);
      } else {
        clearFieldError('bn-phone');
      }
    });
  }
  
  // Ticket booking modal validation
  const tsFname = document.getElementById('ts-fname');
  const tsLname = document.getElementById('ts-lname');
  const tsPhone = document.getElementById('ts-phone');
  
  if (tsFname) {
    tsFname.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^a-zA-Z\s]/g, '');
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('ts-fname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('ts-fname', error);
      } else {
        clearFieldError('ts-fname');
      }
    });
  }
  
  if (tsLname) {
    tsLname.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^a-zA-Z\s]/g, '');
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('ts-lname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('ts-lname', error);
      } else {
        clearFieldError('ts-lname');
      }
    });
  }
  
  if (tsPhone) {
    tsPhone.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^0-9+]/g, '');
      const normalized = cleaned[0] === '+' ? '+' + cleaned.slice(1).replace(/\+/g, '') : cleaned.replace(/\+/g, '');
      if (normalized !== this.value) {
        this.value = normalized;
        showFieldError('ts-phone', '⚠️ Should Enter Only Number.');
        return;
      }
      const digits = this.value.replace(/\D/g, '');
      if (digits.length > 0 && digits.length < 10) {
        showFieldError('ts-phone', '⚠️ Enter 10 Phone Number.');
        return;
      }
      const error = validatePhone(this.value.trim());
      if (error) {
        showFieldError('ts-phone', error);
      } else {
        clearFieldError('ts-phone');
      }
    });
  }
  ...
  if (coFname) {
    coFname.addEventListener('input', function() {
      const cleaned = this.value.replace(/[^a-zA-Z\s]/g, '');
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('co-fname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('co-fname', error);
      } else {
        clearFieldError('co-fname');
      }
    });
  }
  ...
  if (coLname) {
    coLname.addEventListener('input', function() {
      ...
      if (cleaned !== this.value) {
        this.value = cleaned;
        showFieldError('co-lname', '⚠️ Should Enter Alphabet only.');
        return;
      }
      const error = validateName(this.value.trim());
      if (error) {
        showFieldError('co-lname', error);
      } else {
        clearFieldError('co-lname');
      }
    });
  }
  ...
  if (coPhone) {
    coPhone.addEventListener('input', function() {
      ...
      if (normalized !== this.value) {
        this.value = normalized;
        showFieldError('co-phone', '⚠️ Should Enter Only Number.');
        return;
      }
      const error = validatePhone(this.value.trim());
      if (error) {
        showFieldError('co-phone', error);
      } else {
        clearFieldError('co-phone');
      }
    });
  }
}
